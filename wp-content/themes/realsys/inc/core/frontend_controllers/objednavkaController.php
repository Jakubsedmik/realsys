			...
			if($payment == "visa"){
				...
				if(Tools::checkPresenceOfParam($credits, $cenik) || $serviceOrder!= false){
					...
					if(uzivatelClass::getUserLoggedId() == false) {
						...
						$redirectBack = Tools::getFERoute("objednavkaClass",false, "detail") . "?action=processPayment&payment=visa&credits={$credits}&serviceOrder={$serviceOrder}";
						$redirectBack = urlencode($redirectBack);
						Tools::jsRedirect(Tools::getFERoute("uzivatelClass", $redirectBack ,"loginR"));
						$this->setView("error");
						return false;
					}
